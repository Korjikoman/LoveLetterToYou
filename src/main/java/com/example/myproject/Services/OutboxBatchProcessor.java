...
public class OutboxBatchProcessor {
    ...
    public OutboxBatchProcessor(OutboxEventRepository outboxEventRepository, RedisRepository redisRepository, OutboxBackoff outboxBackoff, OutboxLeaseService leaseService, Clock clock) {
        this.clock = clock;
    ...
    private Boolean processEvent(ProcessingOutboxEvent event) {
        switch (event.eventType()) {
            case LETTER_CACHE_EVICT: {
                return redisRepository.evictLetter(event.aggregateId(), event.aggregateEmail());
            }
            default:
                return false;
        }
